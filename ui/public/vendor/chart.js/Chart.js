...
(function(global, factory) {
  ...
  function xyz2rgb(xyz) {
    ...
    // assume sRGB
    r =
      r > 0.0031308 ? 1.055 * Math.pow(r, 1.0 / 2.4) - 0.055 : (r = r * 12.92);

    g =
      g > 0.0031308 ? 1.055 * Math.pow(g, 1.0 / 2.4) - 0.055 : (g = g * 12.92);

    b =
      b > 0.0031308 ? 1.055 * Math.pow(b, 1.0 / 2.4) - 0.055 : (b = b * 12.92);
  ...
  var controller_doughnut = core_datasetController.extend({
    ...
    _getRingWeight: function(dataSetIndex) {
      return Math.max(
        valueOrDefault$4(this.chart.data.datasets[dataSetIndex].weight, 1),
        0
      );
    },
  ...
  var core_layouts = {
    ...
    update: function(chart, width, height) {
      ...
      function finalFitVerticalBox(box) {
        var minBoxSize = helpers$1.findNextWhere(minBoxSizes, function(
          minSize
        ) {
  ...
  var scale_linear = scale_linearbase.extend({
    ...
    getValueForPixel: function(pixel) {
      ...
      var offset =
        (isHorizontal ? pixel - me.left : me.bottom - pixel) / innerDimension;
  ...
  function drawPointLabels(scale) {
    ...
    if (ctx.setLineDash) {
      ctx.setLineDash(
        resolve$7([angleLineOpts.borderDash, gridLineOpts.borderDash, []])
      );
  ...
  var scale_time = core_scale.extend({
    ...
    buildTicks: function() {
      ...
      me._unit =
        timeOpts.unit ||
        determineUnitForFormatting(me, ticks, timeOpts.minUnit, me.min, me.max);
        ...
        generateLabels: function(chart) {
          ...
          return helpers$1.isArray(data.datasets)
            ? data.datasets.map(function(dataset, i) {
                return {
                  text: dataset.label,
                  fillStyle: !helpers$1.isArray(dataset.backgroundColor)
      ...
      if (display) {
        ...
        if (isHorizontal) {
          ...
          helpers$1.each(me.legendItems, function(legendItem, i) {
            ...
            if (
              i > 0 &&
              currentColHeight + itemHeight > minSize.height - vPadding
            ) {
      ...
      if (opts.display) {
        ...
        var drawLegendBox = function(x, y, legendItem) {
          ...
          if (ctx.setLineDash) {
            // IE 9 and 10 do not support line dash
            ctx.setLineDash(
              valueOrDefault$d(legendItem.lineDash, lineDefault.borderDash)
            );
          }
